...
band('MMU BLOCK MAP - byte address = block x $2000 + offset; K2 and Jr2 rc18')
...
add(['WILDBITS | K2 + Jr2 | QUICK REFERENCE | 25 September 2026'],'E0E0E0',True,True)
...
'Sources: rc18 MMU and K2/Jr2 I/O RTL; SID_OPL3_Interface.v fixed sound decode; wildbits.d.']:
 add([s],'E6E6E6',span=True)
t=Table(data,colWidths=widths)
t.setStyle(TableStyle(commands+[('GRID',(0,0),(-1,-1),.8,HexColor('#333333')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('LEFTPADDING',(0,0),(-1,-1),2),('RIGHTPADDING',(0,0),(-1,-1),2),('TOPPADDING',(0,0),(-1,-1),0.30),('BOTTOMPADDING',(0,0),(-1,-1),0.30)]))
t.setStyle(TableStyle(section_styles))
tw,th=t.wrap(552,100000)
c=canvas.Canvas(str(P/'wildbits-memory-map.pdf'),pagesize=(612,792))
c.setTitle('WILDBITS | K2 + Jr2 | QUICK REFERENCE | 25 September 2026')
assert th <= 768, f'Table too tall: {th}'
t.drawOn(c,30,792-12-th);c.showPage();c.save()
print('PDF: US Letter, one page; table height:', th)
# PNG is always rasterized from the PDF, never independently drawn.
import subprocess, shutil
renderer = shutil.which('pdftoppm') or str(Path.home()/'.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/Library/bin/pdftoppm.exe')
subprocess.run([renderer, '-r', '200', '-singlefile', '-png', str(P/'wildbits-memory-map.pdf'), str(P/'wildbits-memory-map')], check=True)
print('Rendered wildbits-memory-map.png from the saved PDF.')